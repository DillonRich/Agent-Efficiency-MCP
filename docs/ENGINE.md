...
3. Provider default (cloud only. Local/compat need an explicit model.)
...
| Ollama / vLLM on LAN | `http://192.168.x.x:11434/v1` |
| LM Studio | `http://127.0.0.1:1234/v1` |
| llama.cpp server | `http://127.0.0.1:8080/v1` |

Set `LOCAL_LLM_MODEL` to whatever `ollama list` or LM Studio shows. Key defaults to `local` (many local servers ignore it).

## Workspace context
`gatherWorkspaceContext` sends the rewriter:

- Top-level plus capped recursive path walk (skips `node_modules`, `dist`, …)
- Stack hints, package/manifest summary, README/AGENTS snippets
- Git changed files and recent commits
- Forced-file snippets when `@promptmcp:file` or implicit paths apply
...
| xai | `grok-3-mini` |
| local / openai_compat | required (no silent default) |

## Eval
`npm run eval -- --provider deepseek` (or another configured provider)
